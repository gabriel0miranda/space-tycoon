...
local function applyIntent(ship, dt)
  ...
  if intent.targetAngle ~= nil then
    ...
    -- PD (ajuste se a nave ficar lenta/rápida demais)
    local P = 8.0 -- Força proporcional (quão rápido ela vira)
    local D = 2.5 -- Força derivativa (quão forte ela freia para não passar do alvo)

    local desiredTorque = (da * P - angularVel * D) * inertia

    local maxTorque = mov.angularAcceleration * inertia
    local appliedTorque = math.max(-maxTorque, math.min(maxTorque, desiredTorque))
...
function ShipMovement.update(playerFlagShip, dt)
  ...
  if playerFlagShip.rigidbody and playerFlagShip.rigidbody.body and not playerFlagShip.rigidbody.body:isDestroyed() then
    if not playerFlagShip.disabled then
